...
2. <a href="#bayes">Байесовские алгоритмы классификации</a><br>
	...
	2.4 <td><a href="#LDF">Линейный дискриминант Фишера</a></td>
...
4. <a href="#nepar_reg">Непараметрическая регрессия</a><br>

5. <a href="#lin_reg">Многомерная линейная регрессия</a><br>(пока не готов)

6. <a href="#nelin_reg">Нелинейная регрессия</a><br>
...
![](https://github.com/uhsd22/ML_LABS/blob/master/plug/CodeCogsEqn.png)
	
	
Затем эти выборочные оценки подставим в оптимальный байесовский классификатор. Получим байесовский нормальный классификатор, который называется также подстановочным (plug-in).

При длине выборки, стремящейся к бесконечности, оценки обладают рядом оптимальных свойств: они не смещены, состоятельны и эффективны. Однако оценки, сделанные по коротким выборкам, могут быть не достаточно точными.
...
Карта классификации Plug-in

![](https://github.com/uhsd22/ML_LABS/blob/master/plug/Class_map_plug.png)

- **Возможные варианты разделяющей кривой**
  + Эллипс
  
  ![](https://github.com/uhsd22/ML_LABS/blob/master/plug/el.png)
  
  
  + Парабола
  
  ![](https://github.com/uhsd22/ML_LABS/blob/master/plug/par.png)
  
  
  + Гипербола
  
  ![](https://github.com/uhsd22/ML_LABS/blob/master/plug/gip.png)
  
  ## **<a name="LDF"></a>Линейный дискриминант Фишера**

  В алгоритме линейного дискриминаната Фишера предполагается, что ковариационные матрицы классов равны, откуда следует, что оптимальный байесовский классификитор принимает вид максимизации некоторой линейной функции:
  
  
  ![](https://github.com/uhsd22/ML_LABS/blob/master/LDF/func.png)
...
Карта классификации ирисов Фишера алгоритмом линейного дискриминанта Фишера:
 ...
 <table>
	<tr>
    <td>
      <img src="https://github.com/uhsd22/ML_LABS/blob/master/LDF/bez_lin.png" width="500" heigth="200">
    </td>
    <td><img src="https://github.com/uhsd22/ML_LABS/blob/master/LDF/lin.png" width="500" heigth="200"></td>
	</tr>
<table>
...
sgAdaline <- function(xl, eta = 1, lambda = 1 / 6)
{
  ...
  Если выберем кусочно-линейную функцию
  ![](https://camo.githubusercontent.com/97f2b6593c8f819f61676cb91aefc2a65784a8d4/687474703a2f2f6c617465782e636f6465636f67732e636f6d2f7376672e6c617465783f2535436d61746863616c2537424c2537442533442532382d4d2532395f2b2533442535436d61782532382d4d25324330253239) функцией потерь , то получим классификатор, называющейся правилом Хэбба.
  ...
  ![](https://github.com/uhsd22/ML_LABS/blob/master/ADALINE/hebb_data.png)
  
  Сравнение с ADALINE
  
  ![](https://github.com/uhsd22/ML_LABS/blob/master/ADALINE/ad_hebb.png)
...
# <a name="nepar_reg"></a> <center><b>Непараметрическая регрессия</b></center>

Задачу обучения по прецедентам при Y = R принято называть задачей восстановления регрессиии, а решающую функцию — функцией регрессии.

## Метод наименьших квадратов

Пусть задана модель регрессии — параметрическое семейство функций g(x, α),
где α ∈ R^p — вектор параметров модели. Определим функционал качества аппроксимации целевой зависимости на выборке X^ℓ как сумму квадратов ошибок:

![](https://github.com/uhsd22/ML_LABS/blob/master/reg/SSE.png)
...
![](https://github.com/uhsd22/ML_LABS/blob/master/reg/vek_SSE.png)


Стандартный способ решения этой оптимизационной задачи — воспользоваться необходимым условием минимума. Если функция g(x, α) достаточное число раз
дифференцируема по α, то в точке минимума выполняется система p уравнений относительно p неизвестных:

![](https://github.com/uhsd22/ML_LABS/blob/master/reg/dif_SSE.png)

Непараметрическая регрессия основана на той же идее, что и непараметрическое восстановлении плотности: значение a(x) вычисляется для каждого объекта x по нескольким
ближайшим к нему объектам обучающей выборки (близость определяется согласно функции расстояния).
Рассмотрим самую простую модель регрессии — константу ф(x, α) = α, но при этом каждому объекту xi выборки назначим веса согласновесовой функции wi(x), где x — объект, в котором мы хотим вычислить значение функции регрессии a(x) = ф(x, α).


Чтобы вычислить оптимальное α для произвольного объекта x воспользуемся МНК:

![](https://github.com/uhsd22/ML_LABS/blob/master/reg/min_SSE.png)

Зададим веса таким образом, чтобы они убывали по мере увеличения расстояния до x. Для этого введем ядро — невозрастающую, гладкую, ограниченную функцию K(z),
где z = ρ(x, xi)/h , а h - ширина окна.

![](https://github.com/uhsd22/ML_LABS/blob/master/reg/W_SSE.png)

Приравнивая нулю производную от Q по α, получим формулу ядерного сглаживания Надарая-Ватсона:

![](https://github.com/uhsd22/ML_LABS/blob/master/reg/N_W.png)

Далее о её сходимости:

![](https://github.com/uhsd22/ML_LABS/blob/master/reg/shod.png)

В качестве ядра при ядерном сглаживании чаще всего используют гауссовское:
...
**При решении задачи возникает проблема выбросов**, которая решается понижением веса для объектов с большой величиной ошибки.Отсюда идея: домножить веса
wi(x) на коэффициенты γi= ~K(εi) , зависящие от ядра отличного от K(z).

Заметим, что коэффициенты γi и ошибки εi, зависят от ah, которая, в свою
очередь зависит от γi. Следовательно, можно организовать итерационный
процесс, пока коэффициенты γi не стабилизируются. Как правило, такой
процесс сходится довольно быстро и называется локально взвешенным
сглаживанием (locally weighted scatter plot smoothing, LOWESS).


# <a name="lin_reg"></a> <center><b>Многомерная линейная регрессия</b></center>
...
![Конспект](https://github.com/uhsd22/ML_LABS/tree/master/reg/konsp_lin)

# <a name="nelin_reg"></a> <center><b>Нелинейная регрессия</b></center>

![Конспект](https://github.com/uhsd22/ML_LABS/tree/master/reg/konsp_nelin)
